...
<body class = "background">
    ...
    <div class="container my-5">
            ...
            <div class="col">
                <form id="userTypeForm">
                    <div class="btn-group" role="group" aria-label="navi">
                        ...
                        <button type="button" class="custom" name="name" id="TeacherBtn" value="Teachers">Teachers</button>
                    </div>
                </form>
            </div>
            ...
            <div class="col">
                ...
                <form class="h2-style" action="" method="get">
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="Search by Teacher ID" name="search" id="search">
        ...
        function displayData($conn, $orderBy, $order) {
            ...
            $sql = "SELECT barcodeid, DATE(date) AS date, TIME(date) AS time FROM attendancetc ORDER BY $orderBy $order";
...
function deleteData($conn, $barcodeId, $date, $time) {
    $sql = "DELETE FROM attendancetc WHERE barcodeid='$barcodeId' AND DATE(date)='$date' AND TIME(date)='$time'";
    if ($conn->query($sql) === TRUE) {
        ...
        function clearAllData($conn) {
            $sql = "TRUNCATE TABLE attendancetc";
            $conn->query($sql);
        }
                    ...
                    <div class="row">
                        <div class="col-md-6">
                                <select class="form-select" id="sortBy" name="orderby">
                                    ...
                                    <option value="barcodeid" ' . (isset($_GET['orderby']) && $_GET['orderby'] == 'barcodeid' ? 'selected' : '') . '>Teacher ID</option>
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $searchSql = "SELECT barcodeid, DATE(date) AS date, TIME(date) AS time FROM attendancetc WHERE barcodeid = '$searchInput'";
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    $deleteMessage = deleteData($conn, $teacherIdToDelete, $dateToDelete, $timeToDelete);
